...
export class NotificationComponent implements OnInit, OnDestroy {
    ...
    try {
      ...
      this.stompClient.connect({},
        (frame: any) => {
          ...
          // Subscribe to general notifications (immediate broadcast)
          this.stompClient.subscribe('/topic/notifications', (message: any) => {
            console.log(`✅ [Component] Received from /topic/notifications: ${message.body}`);
          ...
          this.stompClient.subscribe('/user/queue/notifications', (message: any) => {
            console.log(`✅ [Component] Received from /user/queue/notifications: ${message.body}`);
            // Parse and handle the user-specific notification
            try {
              const notification = JSON.parse(message.body);
  ...
  private handleNotification(notification: any) {
    // Handle general notifications (like category creation)
    console.log('General notification:', notification);
    // Update your UI, show toast, etc.
    this.notifications.unshift(notification);
    this.applyFilter();
    this.unreadCount++;
  }

  private handleUserNotification(notification: any) {
    // Handle user-specific notifications
    console.log('User notification:', notification);
    // Update notification count, show in notification panel, etc.
  }
  ...
  ngOnInit(): void {
    ...
    // Keep the manual connect as backup/testing method
    this.connect();
  }
  ...
  getNotificationIcon(type: string): string {
    switch (type) {
      case 'article':
      case 'ARTICLE_CREATED': return 'bi-file-text';
      case 'category':
      case 'CATEGORY_CREATED': return 'bi-folder';
  ...
  getNotificationColor(type: string): string {
    switch (type) {
      case 'article':
      case 'ARTICLE_CREATED': return 'text-primary';
      case 'category':
      case 'CATEGORY_CREATED': return 'text-success';
  ...
  getNotificationBadgeClass(type: string): string {
    switch (type) {
      case 'article':
      case 'ARTICLE_CREATED': return 'badge bg-primary';
      case 'category':
      case 'CATEGORY_CREATED': return 'badge bg-success';
  ...
  // Manual WebSocket controls for testing/backup
  manualConnect(): void {
    this.connect();
  }
